...
<iframe
  src="https://view.officeapps.live.com/op/embed.aspx?src=https://devexpgbb.github.io/copilot-metrics-roi-toolkit/exec-templates/artifacts/ai-coding-kpi-table-template.docx"
  width="100%"
  height="480"
  frameborder="0"
...
[:material-download: Download DOCX](artifacts/ai-coding-kpi-table-template.docx){ .md-button }
</div>
...
## KPI Dashboard — AI Coding Agent

*Reporting period: [START DATE] — [END DATE]*

| KPI | Executive question | Definition | Source of record | Baseline | Current | Target / threshold | Status / trend | Commentary |
|-----|--------------------|------------|------------------|----------|---------|--------------------|----------------|------------|
| Seat Utilization | Are paid licenses being used? | Assigned seats with AI tool activity in the last 28 days ÷ assigned seats | Vendor usage dashboard / API (e.g., GitHub Copilot Usage Metrics API) | [X]% | [Y]% | 🟢 ≥75%; 🟡 60-74%; 🔴 <60% | [🟢/🟡/🔴] [↑/↓/→] | [e.g., enablement focus for teams below 50% active use] |
| Monthly Active Users (MAU) | Is adoption broadening? | Unique users with AI tool activity in the reporting window | Vendor usage dashboard / API (e.g., GitHub Copilot Usage Metrics API) | [X] | [Y] | Trend up quarter over quarter | [🟢/🟡/🔴] [↑/↓/→] | [e.g., growth came from newly onboarded backend teams] |
...
| Acceptance Rate | Are suggestions relevant enough to use? | Accepted suggestions ÷ shown suggestions for IDE completions | Vendor usage dashboard / editor telemetry export | [X]% | [Y]% | Track trend; investigate sustained drops >5 pts | [🟢/🟡/🔴] [↑/↓/→] | [e.g., lower rate may reflect new languages or prompt quality gaps] |
| Chat Engagement | Are developers using the AI tool beyond completions? | Chat requests per active user per day or week | Vendor usage dashboard / chat metrics export | [X] | [Y] | Increase after enablement; segment by role | [🟢/🟡/🔴] [↑/↓/→] | [e.g., attach examples for test generation, explanation, and migration work] |
| Agent Adoption % | Are teams adopting higher-leverage workflows? | Active users using agent, edit, or multi-file assistance ÷ active users | Code generation dashboard / vendor feature telemetry | [X]% | [Y]% | 🟢 ≥25%; 🟡 10-24%; 🔴 <10% | [🟢/🟡/🔴] [↑/↓/→] | [e.g., prioritize teams with repeatable modernization or refactoring work] |
| Lines Added with AI | Is the AI tool contributing to code throughput? | Accepted AI-generated lines added during the reporting window | Code generation dashboard / vendor usage metrics API | [X] | [Y] | Trend with quality guardrails; avoid using alone | [🟢/🟡/🔴] [↑/↓/→] | [e.g., pair with PR cycle time and change failure rate before claiming impact] |
| PR Cycle Time | Is delivery flow improving? | Median elapsed time from PR open to merge | Source-control API (GitHub, GitLab, Bitbucket, Azure DevOps), Apache DevLake, or BI warehouse | [X] hrs | [Y] hrs | 🟢 ≥15% lower than baseline; 🟡 5-14%; 🔴 flat/worse | [🟢/🟡/🔴] [↑/↓/→] | [e.g., faster cycle time in high-adoption teams supports impact story] |
...
| Developer Satisfaction | Is the AI tool improving developer experience? | Average survey score on usefulness, flow, or time saved | Internal pulse survey | [X]/5 | [Y]/5 | 🟢 ≥4.0/5; 🟡 3.5-3.9; 🔴 <3.5 | [🟢/🟡/🔴] [↑/↓/→] | [e.g., cite top verbatim theme, not individual comments] |
...
| Premium / Paid-Feature Utilization *(GitHub Copilot example below; check vendor for equivalents — e.g., Cursor "fast/slow" requests, Codeium credits)* | Are advanced features scaled sustainably? | Premium requests used ÷ included or budgeted quota | Vendor admin reporting / billing export (Copilot example: Copilot admin reporting) | [X]% | [Y]% | 🟢 <75%; 🟡 75-89%; 🔴 ≥90% | [🟢/🟡/🔴] [↑/↓/→] | [e.g., review heavy-use teams before renewal or quota decisions] |
...
- Choose a consistent baseline period (e.g., the month before AI tool rollout, or your first full month of data).
- If no pre-rollout baseline exists, use your first reporting period as the baseline and track deltas going forward.
...
- For API-sourced metrics, use the toolkit's export or your AI tool vendor's usage endpoints directly (the GitHub Copilot Usage Metrics API is the canonical reference).
...
- Use **vendor dashboards/API** (e.g., GitHub Copilot Usage Metrics API) for adoption, engagement, and feature utilization.
- Use **source control, DevLake, CI/CD, or BI** for delivery and DORA-style outcomes.
...
- Focus on Seat Utilization, License ROI, and Premium / Paid-Feature Utilization.
